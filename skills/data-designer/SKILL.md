...
1. Pick `itemType` from the user's phrasing: `event` (default), `property`, `metric`, `event_variant`, `category`, `property_bundle`, or — to enumerate workspace metadata — `source`, `destination`, `group_type`, `gateway`, `journey`.
